import subprocess, sys; subprocess.check_call([sys.executable, "-m", "pip", "install", "python-telegram-bot", "httpx", "replit", "flask"])
...
if "pending_orders" not in db:
    db["pending_orders"] = json.dumps({})

def load_pending_orders():
    return json.loads(db["pending_orders"])

def save_pending_orders(orders):
    # Сохраняем только нужные поля (без объектов бота)
    to_save = {}
    for uid, order in orders.items():
        to_save[str(uid)] = {
            ...
            "baseline_satoshi": order["baseline_satoshi"],
        }
    db["pending_orders"] = json.dumps(to_save)

pending_orders = load_pending_orders()
def load_products():
    return json.loads(db["products"])

def save_products(products):
    db["products"] = json.dumps(products)
...
async def delete_extra_msgs(context: ContextTypes.DEFAULT_TYPE, chat_id: int):
    """Удалить все дополнительные сообщения (адрес BTC и т.п.)"""
        ...
        keyboard.append(
            [
                InlineKeyboardButton(
                    f"+ {p['name']} ({p['price']} ₽)", callback_data=f"add_{p['id']}"
                )
            ]
        )
        ...
        await query.edit_message_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            "🛍  Перейти в каталог", callback_data="catalog"
                        )
                    ],
                    [HOME_BTN],
                ]
...
async def make_order_btc(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in pending_orders:
        ...
        btc_amount = order["amount_btc"]
        await query.edit_message_text(
            f"⏳ *У вас уже есть активный заказ*\n\n"
            f"Сумма: `{btc_amount:.8f}` BTC\n\n"
            f"Адрес кошелька отправлен отдельным сообщением выше.\n"
            f"Ожидаю поступления средств...",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup(
                [
                    [
    ...
    if rate <= 0:
        await query.edit_message_text(
            "❌ Не удалось получить курс BTC. Попробуйте позже.",
            reply_markup=InlineKeyboardMarkup([[HOME_BTN]]),
        )
        return
    ...
    # Основное сообщение с информацией о заказе
    await query.edit_message_text(
        f"₿ *Оплата Bitcoin*\n\n"
        f"Сумма к оплате:\n"
        f"`{btc_amount:.8f}` BTC\n\n"
        ...
        f"⏳ Бот автоматически проверяет оплату каждые 30 секунд.\n"
        f"Товар будет отправлен сразу после подтверждения.Если оплата будет совершена по истечению 60 минут.Напишите в поддержку админ выдаст товар лично\n"
    ...
    # Адрес — копируется в один тап
    addr_msg = await context.bot.send_message(
        chat_id=user_id,
        text=BITCOIN_ADDRESS,
    )
    context.user_data.setdefault("extra_msgs", []).append(addr_msg.message_id)

    # Инструкция отдельным сообщением — тоже остаётся в чате
    instr_msg = await context.bot.send_message(
        chat_id=user_id,
        text=(
            f"ℹ️ Отправьте точно {btc_amount:.8f} BTC на адрес выше.\n"
            f"Комиссия сети — за ваш счёт.\n"
            f"Бот проверит платёж автоматически."
        ),
    )
    ...
    while True:
        ...
        if time.time() - order["created_at"] > ORDER_TIMEOUT:
            ...
            await application.bot.send_message(
                chat_id=user_id,
                text=(
                    "⌛ Время ожидания оплаты истекло (1 час).\n"
        ...
        if received - baseline >= order["expected_satoshi"]:
            ...
            await application.bot.send_message(
                chat_id=user_id,
                text="✅ *Оплата получена!* Отправляю ваши товары...",
                parse_mode="Markdown",
            )
            for product in order["cart"]:
                text = (
                    product.get("delivery_text")
                    or f"Спасибо за покупку товара «{product['name']}»!"
                )
            ...
            cart_text = "\n".join(
                f"▫ {p['name']} — {p['price']} руб." for p in order["cart"]
            )
            for admin_id in ADMIN_IDS:
                try:
                    await application.bot.send_message(
                        chat_id=admin_id,
                        text=(
                            f"💰 *Новая оплата Bitcoin!*\n\n"
                            f"👤 Покупатель ID: `{user_id}`\n"
                            f"🛒 Товары:\n{cart_text}\n"
                            f"💵 Сумма: {order['amount_rub']} ₽ / `{order['amount_btc']:.8f}` BTC"
                        ),
                        parse_mode="Markdown",
                    )
                except Exception:
...
async def show_my_orders(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elapsed = int(time.time() - order["created_at"])
    remaining = max(0, ORDER_TIMEOUT - elapsed)

    await query.edit_message_text(
        f"📋 *Ваш активный заказ*\n\n"
        f"🛒 Товары:\n{cart_items}\n\n"
        f"💰 Сумма: {total_rub} руб.\n"
        f"₿ К оплате: `{btc_amount:.8f}` BTC\n\n"
        f"👛 *Адрес:*\n`{BITCOIN_ADDRESS}`\n\n"
...
async def show_support(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "samples":
        ...
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("✉️ Написать админу", url=f"tg://user?id={os.getenv('ADMIN_CONTACT_ID')}")],
            [HOME_BTN],
        ])
    else:
        text = (
...
async def adm_add_price(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "Шаг 3/4 — Введите текст, который получит покупатель после оплаты:"
    )
...
async def adm_add_delivery_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "Шаг 4/4 — Отправьте фото для покупателя или нажмите «Пропустить»:",
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        "⏭  Пропустить фото", callback_data="adm_skip_photo"
                    )
                ]
            ]
        ),
...
async def adm_add_delivery_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data["new_product"]["delivery_photo"] = update.message.photo[
        -1
    ].file_id
...
async def _save_new_product(update, context, query=None):
    ...
    if query:
        await query.edit_message_text(
            text, parse_mode="Markdown", reply_markup=admin_main_keyboard()
        )
    else:
        await update.message.reply_text(
            text, parse_mode="Markdown", reply_markup=admin_main_keyboard()
...
async def adm_edit_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not PRODUCTS:
        await query.edit_message_text(
            "Товаров нет.", reply_markup=admin_main_keyboard()
        )
        return ConversationHandler.END
    keyboard = [
        [InlineKeyboardButton(p["name"], callback_data=f"esel_{p['id']}")]
        for p in PRODUCTS
    ]
    keyboard.append([InlineKeyboardButton("🔙 Назад", callback_data="adm_cancel_cb")])
    await query.edit_message_text(
        "✏️ Выберите товар:", reply_markup=InlineKeyboardMarkup(keyboard)
    )
    return EDIT_SELECT
...
async def adm_edit_select(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [
            InlineKeyboardButton(
                "📄 Текст после покупки", callback_data="ef_delivery_text"
            )
        ],
        [
            InlineKeyboardButton(
                "🖼 Фото после покупки", callback_data="ef_delivery_photo"
            )
        ],
...
async def adm_edit_field(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if field == "delete":
        ...
        save_products(PRODUCTS)
        await query.edit_message_text(
            "🗑 Товар удалён.", reply_markup=admin_main_keyboard()
        )
        return ConversationHandler.END
    ...
    prompts = {
        ...
        "delivery_text": "Введите новый текст после покупки:",
        "delivery_photo": "Отправьте новое фото:",
    }
...
async def adm_edit_value_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if field == "price":
        try:
            ...
            await update.message.reply_text(
                "⚠️ Цена должна быть числом. Введите ещё раз:"
            )
...
async def adm_edit_value_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.photo:
        ...
        save_products(PRODUCTS)
        await update.message.reply_text(
            "✅ Фото обновлено!", reply_markup=admin_main_keyboard()
        )
    else:
...
def main():
    ...
    add_conv = ConversationHandler(
        ...
        states={
            ...
            ADD_DELIVERY_TEXT: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, adm_add_delivery_text)
            ],
            ADD_DELIVERY_PHOTO: [
        ...
        fallbacks=[
            ...
            CallbackQueryHandler(adm_cancel_cb, pattern="^adm_cancel_cb$"),
        ],
        per_message=False,
    )
    ...
    edit_conv = ConversationHandler(
        ...
        states={
            ...
            EDIT_FIELD: [
                CallbackQueryHandler(adm_edit_field, pattern="^ef_"),
                CallbackQueryHandler(adm_cancel_cb, pattern="^adm_cancel_cb$"),
            ],
            EDIT_VALUE_TEXT: [
